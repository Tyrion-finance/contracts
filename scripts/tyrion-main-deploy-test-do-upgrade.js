...
const { getImplementationAddress, verifyContract } = require('./utils.js');
...
async function mainETH() {
    ...
    await upgrades.upgradeProxy(registryProxyAddress, TyrionRegistry);
    const registryProxyAddress = getImplementationAddress(registryProxyAddress);

    const TyrionBroker = await ethers.getContractFactory('TyrionBroker');
    await upgrades.upgradeProxy(brokerProxyAddress, TyrionBroker);
    const implAddress = getImplementationAddress(brokerProxyAddress);

    verifyContract(implAddress, []);
    verifyContract(implAddress, [], false);
